...
const command = yargs
  .options({
    ...
    "sep-config": {
      ...
      requiresArg: true,
      description:
        "A relative or absolute file path to JSON file containing the configuration required for SEP 6, 12, 24, 31 & 38.",
    },
  })
  .check((argv: any) => {
    ...
    if (argv.seps) {
      ...
      if (
        (argv.seps.includes(6) ||
          ...
          argv.seps.includes(24) ||
          argv.seps.includes(31) ||
          argv.seps.includes(38)) &&
        !argv.sepConfig
      ) {
        throw "error: SEP 6, 12, 24, 31 & 38 require a configuration file (--sep-config, -c)";
      }
    }
    return true;
  });
