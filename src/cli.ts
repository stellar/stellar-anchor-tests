...
const args = yargs.options({
	...
	"currency": {
		...
		"requiresArg": true,
		"type": "string",
		"description": "The currency to use for testing. Must match one of the CURRENCIES listed in the TOML file."
	},
	"output-format": {
		"alias": "o",
		"requiresArg": true,
		"default": "coloredText",
		"choices": ["coloredText", "text", "markdown", "json"],
		"type": "string",
		"description": "The output format to use when sending content to standard output.",
	},
	"seps": {
		"alias": "s",
		"type": "array",
	...
	"mainnet-master-account-secret": {
		...
		"type": "string",
		"requiresArg": true,
		"description": "The Stellar account to use when when funding temporary test accounts. Currently, 50XLM must be present in the account."
	}
}).check((argv: any) => {
...
(async () => {
	...
	if (args.currency)
		config.currency = args.currency as string;
	if (args.verbose)
		config.verbose = args.verbose as boolean;
